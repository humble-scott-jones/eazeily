...
function normalizeQueueEntry(entry = {}) {
  ...
  return {
    key: entry.key || '',
    platform: normalizePlatformKey(entry.platform || ''),
    platformLabel: entry.platformLabel || formatPlatformLabel(entry.platform || ''),
    day: entry.day || 0,
    pillar: entry.pillar || '',
...
function buildPlatformMetadata(post = {}) {
  const normalized = normalizePlatformKey(post.platform || '');
  ...
  card.innerHTML = `
    ...
    <div class="grid md:grid-cols-3 gap-3 mt-3 mb-2">
      ...
      <div class="text-xs text-slate-600 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
        ...
        <p class="truncate">${escapeHtml(metadata.hashtags.join(' ') || 'None yet')}</p>
      </div>
      <div class="text-xs text-slate-600 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
        <p class="font-semibold text-slate-900">Preview</p>
        <p class="truncate">${escapeHtml(metadata.preview)}</p>
      </div>
    </div>

    <div class="mt-3 border-t border-slate-200 pt-3 space-y-2" data-publish-surface>
      <div class="flex flex-wrap items-center justify-between gap-2">
        <div class="flex items-center gap-2">
          <span class="text-[11px] font-semibold px-2 py-1 rounded-full border ${queueStatus}" data-queue-badge="${queueEntry.key}">${formatQueueStatusLabel(queueEntry.status)}</span>
          <span class="text-xs text-slate-500" data-queue-note="${queueEntry.key}">${formatQueueDetail(queueEntry)}</span>
        </div>
        <div class="flex items-center gap-2">
          <button class="btn-ghost text-xs" data-copy-target="${editorId}" data-stamp-target="${fallbackStampId}">Copy text</button>
          <span id="${fallbackStampId}" class="copy-timestamp"></span>
        </div>
      </div>
      <div class="flex flex-wrap items-center gap-2">
        <button class="btn-primary btn-sm" data-publish-now="${queueEntry.key}">Publish now</button>
        <button class="btn-secondary btn-sm" data-schedule-btn="${queueEntry.key}">Schedule</button>
        <label class="text-xs text-slate-600 flex items-center gap-2">
          <input type="datetime-local" class="input text-xs" data-schedule-input="${queueEntry.key}" aria-label="Schedule time" />
          <span class="text-[11px] text-slate-500">${escapeHtml(getTimezoneLabel() || '')}</span>
        </label>
        <button class="btn-ghost btn-sm ${queueEntry.status === 'failed' ? '' : 'hidden'}" data-retry-btn="${queueEntry.key}">Retry</button>
      </div>
    </div>
  `;
