...
function renderPostCard(post) {
  ...
  card.innerHTML = `
    ...
    <div class="flex flex-wrap gap-2 mb-3">
      <button class="btn-primary text-xs px-3 py-2 flex items-center gap-1" data-copy-combined="${editorId}" ${hashtags.length > 0 ? `data-hashtags-id="${hashtagsId}"` : ''}>
        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" />
        </svg>
        ${hashtags.length > 0 ? 'Copy Caption + Hashtags' : 'Copy Caption'}
      </button>
  ...
  if (copyCombinedBtn) {
    copyCombinedBtn.addEventListener('click', async () => {
      ...
      let combinedText = '';
      if (captionEl) {
        // Trim caption to avoid extra blank lines
        combinedText = captionEl.value.trim();
      }
