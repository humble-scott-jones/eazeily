...
async function refreshCurrentUser(){
  ...
  }catch(e){ setCurrentUser(null); return null; }
}
...
document.addEventListener('DOMContentLoaded', () => {
  // render auth UI immediately so the Sign in link opens the modal even if /api/current_user fails
...
function renderAuthUi(){
  // Prefer an explicit header auth link id for robust targeting across templates
  const link = document.querySelector('#header-auth-link') || document.querySelector('#header-auth-link-mobile');
  if (!link) return;
  // ensure we don't attach duplicate handlers
  link.replaceWith(link.cloneNode(true));
  const newLink = document.querySelector('#header-auth-link') || document.querySelector('#header-auth-link-mobile');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const headerLink = document.querySelector('#header-auth-link') || document.querySelector('#header-auth-link-mobile');
