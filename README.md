...
# Swelly — development guide

This document explains how to set up and run Swelly locally, run tests (including reproducing CI artifact collection), install optional Playwright tooling, and troubleshoot common issues.
...
You don’t need a ChatGPT subscription in the app. Swelly uses the OpenAI API when configured. If an API key isn’t set, it falls back to the built‑in generator.
...
Swelly includes a comprehensive production launch checklist with prioritized, actionable items and GitHub issue templates.
...
Swelly includes an automated system for processing user feedback into GitHub issues. When users submit feedback through the application, it can be automatically:
...
url = "https://api.github.com/repos/humble-scott-jones/swelly/dispatches"
payload = {
