#!/usr/bin/env python3
"""
Safe repo-wide rename tool: togetherly -> swelly
...
- In text files, replaces:
    ...
    Togetherly  -> Swelly
    TOGETHERLY  -> SWELLY
- Renames files and directories whose names contain 'togetherly' (same casing rules)
...
def main():
    parser = argparse.ArgumentParser(description="Rename togetherly -> swelly across a repo (dry-run by default).")
    ...
    if not args.apply:
        ...
        print("Example: python3 rename_togetherly_to_swelly.py --apply")
        sys.exit(0)
    ...
    print(" - Update the SQLite DB filename if you renamed togetherly.db -> swelly.db in code/config.")
