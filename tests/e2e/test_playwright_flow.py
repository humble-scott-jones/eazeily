...
def test_playwright_happy_path(tmp_path):
    ...
    try:
        with sync_playwright() as pw:
            ...
            # click the dashboard generate button if present
            try:
                page.click('#generate-content')
            except Exception:
                # fallback: call API directly
                page.evaluate("() => fetch('/api/generate', {method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({platforms:['instagram'], days:3})})")
