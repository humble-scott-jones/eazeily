...
# Determine DB_PATH with a short compatibility window:
# - If DB_PATH environment variable is set, respect it.
# - Prefer the new `swelly.db` when present.
# - Fallback to the legacy `togetherly.db` if it exists (so CI/runners keep working during transition).
...
def review_response_page():
    ...
    If the specific template isn't present, fall back to the main index to avoid 500s in dev.
    """
    try:
...
def api_stripe_webhook():
    ...
    event = None
    db = get_db()
    # If a real webhook secret is configured (not the placeholder), verify signatures when possible
    if secret and secret != 'whsec_REPLACE_ME' and stripe and sig_header:
        try:
